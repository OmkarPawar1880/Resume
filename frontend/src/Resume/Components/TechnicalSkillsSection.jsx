...
const TechnicalSkillsSection = () => {
  const { resume, replaceSection ,savePartialResumeTo } = useResume();
...
const validateSkills = () => {
  ...
  <div key={catIndex}>
    ...
    {errors[catIndex]?.title && (
  <p className="error-text">
    {errors[catIndex].title}
  </p>
)}

{errors[catIndex]?.skills && (
  <p className="error-text">
    {errors[catIndex].skills}
  </p>
)}

{errors[catIndex]?.skillErrors?.map((skillError, index) => (
  <div key={index}>
    {skillError.name && (
      <p className="error-text">
        Skill {index + 1}: {skillError.name}
      </p>
    )}

    {skillError.level && (
      <p className="error-text">
        Skill {index + 1}: {skillError.level}
      </p>
    )}
  </div>
))}
